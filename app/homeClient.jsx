    ...
    const pickSponsorsFor = useCallback((cafe) => {
      const sponsors = cafes.filter(
        (c) => c.is_sponsored === true && c.id !== cafe.id
      );
      ...
      // shuffle then take up to 3, so repeat opens don't always show the same trio
      const shuffled = [...pool];
...
const openHistoryEntry = (entry) => {
  const fullCafe = cafes.find(
    (c) => String(c.id) === String(entry.id)
  );
