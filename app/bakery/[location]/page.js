...
import { getBakeryLocationConfig } from '@/lib/bakeryLocations';
...
export default async function BestBakeryPage({ params }) {
  const { location } = await params;
  const config = getBakeryLocationConfig(location);
  if (!config) notFound();

  const restaurants = await getEstablishments('bakery', config.query, 30);
      ...
      <div className="mx-auto w-full max-w-3xl">
        <header className="mb-8">
          <h1 className="text-2xl font-bold tracking-tight text-black sm:text-4xl">
            Best Bakeries in {config.name}{config.slug === 'phnom-penh' ? '' : ', Phnom Penh'} (2026)
          </h1>
